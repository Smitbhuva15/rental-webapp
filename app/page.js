      ...
      <section className="py-24 bg-white dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* Category Cards */}
            {['Apartments', 'Villas', 'Houses', 'Commercial'].map((item, i) => (
              <Link href={`/browse?category=${item}`} key={i} className="group relative rounded-2xl overflow-hidden aspect-[4/3]">
                ...
                <div className="absolute bottom-6 left-6 z-20">
                  <h3 className="text-white text-xl font-bold group-hover:text-primary transition-colors">{item}</h3>
                  <p className="text-slate-300 text-sm mt-1 transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">Browse listings ➔</p>
                </div>
              </Link>
            ))}
          </div>
